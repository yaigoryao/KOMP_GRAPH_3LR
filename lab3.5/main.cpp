...
//////////////////////////////////////////////////////////////////
// Применение камеры преобразования данной системы отсчета. Это в //значительной степени альтернативная реализация gluLookAt использующую тип //float вместо double и имеющую прямой вектор вместо указателя.
...
static std::vector<std::vector<GLfloat>> colors = std::vector<std::vector<GLfloat>>(NUM_SPHERES);
void SetupRC()
{
	int iSphere;
	// Голубоватый фон
	...
	{
		colors[i].push_back((std::rand()%101) / 100.f);
		colors[i].push_back((std::rand()%101) / 100.f);
		colors[i].push_back((std::rand()%101) / 100.f);
	}
}
...
static bool drawCubes = true;
// Вызывается для рисования сцены
void RenderScene(void)
{
	glColor3b(127, 127, 127);
	int i;
	static GLfloat xRot = -1.5f, yRot = 2.f, zRot = -2.5f; // Используемый в анимации угол поворота
	xRot += 0.25f;
	yRot += 0.5f;
	zRot -= 1.f;

	static GLfloat thorusFloat = 0.f;
	static GLfloat kettleRot = 0.f;

	kettleRot += 0.3f;
	if (kettleRot >= 360.f) kettleRot = 0.f;
	thorusFloat += 0.002f * (dir ? 1 : -1);
	if (thorusFloat < -.2f) dir = !dir;
	else if (thorusFloat > .2f) dir = !dir;
	...
	for (i = 0; i < NUM_SPHERES; i++)
	{
		...
		glRotatef(kettleRot, 0.f, 1.f, 0.f);
	...
	glTranslatef(0.0f, thorusFloat, 0.0f);
	glPushMatrix();
	...
	glPushMatrix();
	glRotatef(zRot * 2.0f, 1.0f, 0.0f, 0.0f);
	...
	glPopMatrix();
		...
		/*glPushMatrix();
		glTranslatef(0.0f, 1.0f, 0.0f);
		glRotatef(-yRot * 2.0f, 0.0f, 1.0f, 1.0f);
...
void SpecialKeys(int key, int x, int y)
{
	...
	if(key == 16)
		...
		//gltRotateFrameLocalZ(&frameCamera, -0.1f);
...
int main(int argc, char* argv[])
{
	...
	glutCreateWindow("OpenGL Kettle World Demo");
